...
void findRight(int i, int j, Block_Edge startBe, int depth)
{
	...
	for (m = 0; m < 16; m++) {
		for (n = 0; n < 16; n++) {
			if (m == i && n == j) continue;
			if (BE[m][n].book == 0) {
				if ((startBe.nEdgeSum[2] == BE[m][n].nEdgeSum[1]) && (memcmp(&startBe.edgeNum[2], &BE[m][n].edgeNum[1], 256 * sizeof(int)) == 0)) {
...
void findDown(int i, int j, Block_Edge startBe, int depth)
{
	...
	for (m = 0; m < 16; m++) {
		for (n = 0; n < 16; n++) {
			if (m == i && n == j) continue;
			if (BE[m][n].book == 0) {
				if ((startBe.nEdgeSum[3] == BE[m][n].nEdgeSum[0]) && (memcmp(&startBe.edgeNum[3], &BE[m][n].edgeNum[0], 256 * sizeof(int)) == 0)
					|| (startBe.nEdgeSum[3] == BE[m][n].nEdgeSum[4]) && (memcmp(&startBe.edgeNum[3], &BE[m][n].edgeNum[4], 256 * sizeof(int)) == 0)
					|| (startBe.nEdgeSum[7] == BE[m][n].nEdgeSum[0]) && (memcmp(&startBe.edgeNum[7], &BE[m][n].edgeNum[0], 256 * sizeof(int)) == 0)
					|| (startBe.nEdgeSum[7] == BE[m][n].nEdgeSum[4]) && (memcmp(&startBe.edgeNum[7], &BE[m][n].edgeNum[4], 256 * sizeof(int)) == 0)) {
...
void test(unsigned char map[N4096][N4096])
{
	...
	for (i = 1; i < 16; i++)
	{
		be = BE[BI[0][i].X][BI[0][i].Y];
		findDown(BI[0][i].X, BI[0][i].Y, be, 0);
	}
	...
	for (i = 0; i < 16; i++) {
		for (j = 0; j < 16; j++) {
			...
			{
				memcpy(&tempmap[(y2*N256 + cy) * 4096 + x2 * N256], &map[y1*N256 + cy][x1*N256], N256);
			}
		}
	}

	memcpy(map, tempmap, 4096 * 4096);
	free(tempmap);
	tempmap = NULL;
}